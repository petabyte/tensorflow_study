...
with tf.variable_scope("lstm"):
    cell = rnn.MultiRNNCell(cells=[rnn.BasicLSTMCell(hidden_layer_size,forget_bias=1.0) for _ in range(num_LSTM_layers)], state_is_tuple=True)
    outputs, states = tf.nn.dynamic_rnn(cell, embed, sequence_length = _seqlens,dtype=tf.float32)
    # lstm_cell = rnn.BasicLSTMCell(hidden_layer_size, forget_bias=1.0)
